...
def main(exp_dir, last_model_path, pseudoseg_save_path, pseudodet_save_path):

    print('============' + exp_dir + '============')

    gtpointlab_save_path = os.path.join(exp_dir, 'gtpoint_label')
    generate_sparse_plabel_v1(args.data_dir_target_point, pseudoseg_save_path, gtpointlab_save_path)  # 0,1,255
    ...
    targetloader = data.DataLoader(
        targetDataSet_train_step2_full(args.data_dir_target, gtpointlab_save_path, args.data_list_target, 
                                  max_iters=args.num_steps, iter_start=args.iter_start, 
    ...
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.9, patience=2000, verbose=True)
    ...
    for i_iter in range(args.iter_start, args.num_steps+1):
        ...
        images, flabels, points, gaussians, _ = batch
        if usecuda:
        ...
        loss_det_target = weight_mse(det_target, gaussians, args.gpu)
        ...
        if i_iter % 500 == 1:
            ...
            obj_queue_target_negative, bck_queue_target_negative = \
                generate_target_proto_stage2_up(feature.detach(), points, flabels, 
                                                      obj_queue_target_positive,bck_queue_target_positive, 
        ...
        loss = 0.001*loss_det_target + 1*loss_seg_target + 0.005*loss_contrast_target
        ...
        if (i_iter % 50 == 0):
            ...
            print('iter = {0:8d}/{1:8d}, loss_seg_ce_target = {2:.5f}, loss_seg_dice_target = {2:.5f},'
...
if __name__ == '__main__':
    ...
    exp_base_name = 'cvlab2R_100%_stage2'
    last_exp_dir =  "./WSDA_cvlab2R/cvlab2R_0%_stage1/iter0"
    ...
    args.data_dir_target_point = "/media/icml005/F/xs/data/MitoEM-R-new1/parttrain/100%_split_resize"

    code_path_list= ['./02_Stage2_WSDA-full_final.py', './add_arguments.py', './model/HSC82.py',
    ...
    for iter in range(1, 3):
        ...
        # if iter == 1:
        #     last_model_path = os.path.join(last_exp_model_dir, 'best_20000.pth')
        # else:
        ...
        # point_num = val_threshold(last_model_path, pseudoseg_save_path, pseudodet_save_path, args, threshold=0.9)
       
        print('============Generate Pseudo Label Start============')

        main(exp_dir, last_model_path, pseudoseg_save_path, pseudodet_save_path)
