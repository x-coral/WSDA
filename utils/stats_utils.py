...
def dice_coeff_checkforPoint(pred, target, point):
    pred_blobs = get_blobs(pred)

    p = point.squeeze().cpu().numpy()
    fg_unique = np.unique(pred_blobs * p)
    bg_unique = [x for x in np.unique(pred_blobs) if x not in fg_unique]

    for u in bg_unique:
        pred_blobs[pred_blobs == u] = 0

    mask = pred_blobs > 0
    pred = pred * mask

    target = target.squeeze().cpu().numpy()
    target_blobs = get_blobs(target)

    fg_unique = np.unique(target_blobs * p)
    bg_unique = [x for x in np.unique(target_blobs) if x not in fg_unique]

    for u in bg_unique:
        target_blobs[target_blobs == u] = 0
...
if __name__ == '__main__':
    pred_partiallabel = '../pr-vnc2cvlab-D+Det+Count+VNC-BackgroundThresold=0.10+ST-Iter50000-dropconsistencyloss/testoutput/seg_postprocess'
    gt_partiallabel = '/media/peng/F/qdf/data/cvlabdata/test/lab/'
