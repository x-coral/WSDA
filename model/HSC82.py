...
class CoDA_Net(nn.Module):
    def __init__(self, in_channels, out_channels, device, has_dropout=False):
        ...
        # self.Conv_up2_det = Conv_up(576, 288)
        # self.Conv_up3_det = Conv_up(288, 144)
        # self.Conv_up4_det = Conv_up(144, 72)
        self.conv1_1 = nn.Conv2d(144, 144, 1)
        self.conv1_2 = nn.Conv2d(144, 128, 1)
        self.Conv_up3_feature = Conv_up(288, 144)
        self.norm1 = nn.InstanceNorm2d(144)
        self.norm2 = nn.InstanceNorm2d(128)

        self.Conv_up5 = Double_conv(72, 72)
        self.Conv_up6 = Double_conv(72, 72)
        self.Conv_up7 = Double_conv(72, 72)
        self.Conv_up8 = Double_conv(72, 72)
        self.Conv_up9 = Double_conv(72, 72)
    ...
    def forward(self, x):
        ...
        # det = self.Conv_up2_det(x, conv3)
        # det = self.Conv_up3_det(det, conv2)
        # det = self.Conv_up4_det(det, conv1)

        feature = self.Conv_up2(x, conv3)
        x = self.Conv_up3(feature, conv2)
        x = self.Conv_up4(x, conv1)


        feature = self.Conv_up3_feature(feature, conv2)
        ...
        seg_x1 = self.up(seg_x)
        seg_x = self.Conv_out(seg_x1)

        det_x = self.Conv_up6(x)
        det_x = self.Conv_up7(det_x)
        det_x = self.Conv_up8(det_x)
        det_x = self.Conv_up9(det_x)
        det_x = self.up_c(det_x)
        det_x = self.Conv_out_c(det_x)
        det_x = self.act(det_x)

        return seg_x, det_x, feature
