...
def main(exp_dir, last_model_path, pseudoseg_save_path, pseudodet_save_path, ratio0, ratio):
    ...
    maxpointlab_save_path = os.path.join(exp_dir, 'maxpoint_label')
   
    print('============From det Result Generate Partial Pseudo Label Start============')
    from_detectionmap_generate_pseudolab_v2(pseudoseg_save_path, pseudodet_save_path, maxpointlab_save_path, ratio0,
    ...
    targetloader = data.DataLoader(
        targetDataSet_train_step1_uda(args.data_dir_target, maxpointlab_save_path, partial_pseudolabel_save_path, pseudodet_save_path,
    ...
    testloader = data.DataLoader(
        counting_testdatset(args.data_dir_target, args.data_dir_target_label, './dataset/MitoEMH_list/parttrain11.txt',
                            sigma=args.sigma, batch_size=args.batch_size),
        batch_size=1, shuffle=False)
    
    model.load_state_dict(torch.load(last_model_path, map_location="cuda:" + str(args.gpu)), strict=False)
    ...
    for i_iter in range(args.iter_start, args.num_steps+1):
        ...
        loss = 0.2*loss_det_target+ 1*loss_contrast_target 
...
if __name__ == '__main__':
    ...
    exp_root_dir = './WSDA_R2H/'
    exp_base_name = 'R2H_0%_stage1'
    last_exp_dir =  "./pretrain_model/R2H_full_seg_contrast_det.pth"
    
    args.data_dir_target_point = "/data/data/MitoEM-R-new1/parttrain/0%_split_resize"
    ...
    code_path_list= ['./01_Stage1_WSDA-UDA_final.py', './add_arguments.py', './model/HSC82.py',
    ...
    print('load segmentation model from:', last_exp_dir)
    model.load_state_dict(torch.load(last_exp_dir, map_location="cuda:" + str(args.gpu)), strict=False)
    ...
    for iter in range(0,5):
        ...
        # last_model_path = os.path.join(last_exp_model_dir, 'model10000.pth')
        ...
        label_selected_ratio0 = 0.15
        label_selected_ratio = (iter+1) * 0.15
        if iter == 0:
            last_model_path = last_exp_dir

        print('============Generate Pseudo Label Start============')

        point_num0 = val_threshold(last_model_path, pseudoseg_save_path, pseudodet_save_path, args, threshold=0.9)
        # if iter == 0:
        #     point_num0 = point_num
             
        print('============Generate Pseudo Label Start============')
        
        main(exp_dir, last_model_path, pseudoseg_save_path, pseudodet_save_path, label_selected_ratio0, label_selected_ratio)
